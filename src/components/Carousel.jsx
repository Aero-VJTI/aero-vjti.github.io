...
export default function Carousel({
  ...
  if (length === 0) {
    return (
      <div className="w-100 bg-gray-100 flex items-center justify-center rounded-xl">
  ...
  return (
    <div
      className={`relative w-[90%] md:w-[80%] overflow-hidden rounded-2xl shadow-lg ${className}`}
          ...
          {images.map((_, i) => (
            <button
              ...
              className={`w-3 h-3 rounded-full bg-white transition ${
                i === index ? "scale-125 opacity-100" : "opacity-60"
              }`}
            />
          ))}
        </div>
